...
const MagazineTab = ({ role }) => {
  ...
  const fetchFolders = async () => {
    try {
      const res = await fetch(`${API_URL}/public/api/magazine-folder/get`, {
        method: "POST",
      ...
      console.error(err);
      showToast("Failed to fetch magazine folders", "danger");
    }
  };
  ...
  const fetchFiles = async (folderId) => {
    try {
      const res = await fetch(`${API_URL}/public/api/magazine-folder/${folderId}/magazine-file/list`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      ...
      console.error(err);
      showToast("Failed to fetch magazine files", "danger");
    }
  };
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/public/api/magazine-folder/create`, {
        method: "POST",
  ...
  const handleUpdateFolder = async () => {
    try {
      const res = await fetch(`${API_URL}/public/api/magazine-folder/update/${selectedFolder.id}`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ name: editFolderName, view_only: editViewOnly }),
      });
      if (res.ok) {
        setFolders(folders.map(f => f.id === selectedFolder.id ? { ...f, name: editFolderName, view_only: editViewOnly } : f));
  ...
  const toggleViewOnly = async (folder) => {
    try {
      const res = await fetch(`${API_URL}/public/api/magazine-folder/update/${folder.id}`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
  ...
  const handleDeleteFolder = (id) => {
    confirmPopup("Delete this folder and all its files?", async () => {
      try {
        await fetch(`${API_URL}/public/api/magazine-folder/delete/${id}`, {
          method: "POST",
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleUpload = async (e) => {
    e.preventDefault();
    if (!file || !selectedFolder) return showToast("Select a folder and file first", "danger");

    const formData = new FormData();
    formData.append("file", file);

    try {
      const res = await fetch(`${API_URL}/public/api/magazine-folder/${selectedFolder.id}/magazine-file/upload`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      });
      ...
      console.error(err);
      showToast("Server error during file upload", "danger");
    }
  };
  ...
  const handleDeleteFile = (id) => {
    confirmPopup("Delete this file?", async () => {
      try {
        await fetch(`${API_URL}/public/api/magazine-file/${id}/delete`, {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        });
        fetchFiles(selectedFolder.id);
  ...
  const handlePreview = async (file) => {
    try {
      const res = await fetch(`${API_URL}/public/api/magazine-file/${file.id}/download`, {
        method: "GET",
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    <div className="container-fluid">
      ...
      <div className="d-flex justify-content-between align-items-center flex-wrap mb-3">
        <h4 className="mb-0">
          {selectedFolder ? (
            <>
              <Button variant="link" size="sm" id="back-btn" className="p-0 me-2 text-decoration-none" onClick={handleBackToFolders}>
                <FaArrowLeft className="me-1" /> Back to Folders
              </Button>
        ...
        {!selectedFolder && role === "admin" && (
          <span className="create-folder" onClick={() => setShowFolderModal(true)}>+ Create Folder</span>
        )}
      </div>
      ...
      {selectedFolder && (
        <div className="mt-3">
          ...
          <Modal show={showPreviewModal} onHide={() => setShowPreviewModal(false)} size="lg" centered>
            <Modal.Header closeButton><Modal.Title>Preview PDF</Modal.Title></Modal.Header>
            <Modal.Body>
      ...
      <Modal show={showFolderModal} onHide={() => setShowFolderModal(false)}>
        <Modal.Header closeButton><Modal.Title>Create Folder</Modal.Title></Modal.Header>
        <Modal.Body>
          <Form.Group>
      ...
      <Modal show={showEditModal} onHide={() => setShowEditModal(false)}>
        <Modal.Header closeButton><Modal.Title>Edit Folder</Modal.Title></Modal.Header>
        <Modal.Body>
          <Form.Group>
            <Form.Label>Folder Name</Form.Label>
